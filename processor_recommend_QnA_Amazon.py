...
if __name__ == "__main__":
    ...
    #laptop_data["Graphic card RAM No"] =  laptop_data["Graphics Card Ram Size"].apply(lambda x: get_first_or_none(numbers_extractor(str(x))))
    laptop_data["Graphic card RAM No"] =  laptop_data["Graphics Card Ram Size"].apply(lambda x: get_first_or_none(numbers_extractor(str(x))))
    ...
    laptop_data["Display size"] = laptop_data["Standing screen display size"].apply(screen_size)
    laptop_data["Processor Speed Num"] = laptop_data["Processor Speed"].apply(processor_speed)
    laptop_data["Max Memory Support"] =  laptop_data["Maximum Memory Supported"].apply(lambda x: get_first_or_none(numbers_extractor(str(x))))


    df = laptop_data[["Brand", "Series", "Weight Num", "RAM Num", "price","Graphic card RAM No", "Disk Num", "Battery Life","Display size", "Processor Speed Num", "Max Memory Support", "product url" ]]
    df.to_csv("processed_product_data")
    df.to_excel("processed_product_data.xlsx")

    product_features = df[["Weight Num", "RAM Num", "Graphic card RAM No", "Disk Num", "Battery Life", "price"]]
    product_features_unchanged = product_features



    user_vec = [np.nan for i in range(12)]
    ...
    inp = input()
    user_vec = update_user_vector(user_vec, q4_options[inp]["change"])
